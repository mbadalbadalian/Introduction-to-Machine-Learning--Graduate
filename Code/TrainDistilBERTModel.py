...
def CreateDistilBERTModelFineTuned(model,train_loader_fine_tuned,test_loader_fine_tuned,training_args,ditilBERT_model_fine_tuned_filepath):
    ...
    if checkpoints:
        ...
        print(f"Resuming training from epoch {start_epoch + 1} with best validation loss: {best_test_loss}")
    ...
    for epoch in range(start_epoch, start_epoch + training_args.num_train_epochs):
        ...
        print(f'Epoch {epoch + 1}/{start_epoch + training_args.num_train_epochs}, Validation Loss: {average_test_loss}')
        ...
        if average_test_loss < best_test_loss:
            ...
            torch.save(checkpoint, os.path.join(checkpoint_filepath, f'checkpoint_epoch_{epoch + 1}.pt'))
...
if __name__ == "__main__":
    ...
    create_or_load_string_train_and_test_loader_fine_tuned = 'Load'
    
    #Main Code
    training_args = TrainingArguments(
        output_dir=ditilBERT_model_fine_tuned_filepath,
        num_train_epochs=5,
        per_device_train_batch_size=8,
        per_device_etest_batch_size=8,
        warmup_steps=500,
        weight_decay=0.01,
        logging_dir='./logs',
        save_total_limit=3,
