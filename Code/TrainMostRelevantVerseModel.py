...
def CreateOrLoadTrainAndTestLoader(prepared_Q_and_A_DF,model,train_loader_filepath,test_loader_filepath,create_or_load_string_train_and_test_loader='load'):
    if create_or_load_string_train_and_test_loader.lower() in ['create', 'c']:
...
def collate_fn(batch):
    ...
    target_embeddings = [sample['target_embedding'] for sample in batch]  # Change this line
    ...
    return {'verses': verses, 'packed_input_ids': packed_input_ids, 'packed_attention_masks': packed_attention_masks, 'target_embedding': target_embeddings}
    ...
    for epoch in range(start_epoch,num_epochs):
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                target_embeddings = torch.stack(target_embeddings).squeeze(1)  # Squeeze to remove the extra dimension
